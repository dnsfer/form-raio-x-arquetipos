 const BIA_IG = "https://instagram.com/biancarodrigues.sm";
      ...
      const ARC = {
        ...
        sabio: {
          ...
          forte:
            "Autoridade percebida — quando fala, as pessoas param para ouvir.",
        ...
        mago: {
          ...
          forte:
            "Cria lealdade intensa — clientes viram evangelistas da marca.",
          desafio:
            "Precisa entregar o que promete, ou a credibilidade não resiste.",
          ex: "Apple · Disney · TED",
        ...
        pessoaComum: {
          ...
          forte:
            "Empatia instantânea — as pessoas se sentem entendidas de verdade.",
          desafio:
            "Pode perder diferenciação se não tiver um ponto de vista único.",
        ...
        comediante: {
          ...
          desafio:
            "O humor precisa ser inteligente para não banalizar a marca.",
        ...
        cuidador: {
          ...
          desafio:
            "Pode criar dependência emocional se não empoderar o cliente.",
...
<div style="animation:up .38s ease">
  ...
  <div style="margin-top:52px;display:flex;gap:18px;flex-wrap:wrap">
    ${["◯", "✦", "⬡", "♡", "◈", "✿", "△", "☓"].map((s) => `<span style="font-size:1.6rem;color:var(--y);opacity:.18">${s}</span>`).join("")}
  </div>
</div>`;
        } else if (phase === "info") {
          r.innerHTML = `
<div style="animation:up .38s ease">
  ...
  <button class="btn fw" id="cont" ${!name.trim() ? "disabled" : ""} onclick="doInfo()" style="margin-top:8px">CONTINUAR →</button>
</div>`;
...
<div style="animation:up .3s ease">
  <div class="prog-wrap">
    <div class="prog-meta"><span class="tiny">PERGUNTA ${String(qi + 1).padStart(2, "0")} / ${QS.length}</span><span class="tiny">${name.toUpperCase()}</span></div>
    <div class="prog-track"><div class="prog-fill" style="width:${pct}%"></div></div>
  </div>
  <h2 style="font-size:clamp(1.15rem,3.5vw,1.55rem);font-weight:400;line-height:1.4;color:var(--c);margin-bottom:24px">${q.q}</h2>
  <div>${q.opts.map((o, i) => `<button class="opt${sel === i ? " sel" : ""}" onclick="setSel(${i})"><span class="opt-lt">${String.fromCharCode(65 + i)}.</span>${o.t}</button>`).join("")}</div>
  <div style="display:flex;justify-content:flex-end;margin-top:10px">
    <button class="btn" id="nxt" ${sel === null ? "disabled" : ""} onclick="next()">${qi < QS.length - 1 ? "PRÓXIMA →" : "VER RESULTADO →"}</button>
  </div>
</div>`;
        } else if (phase === "calc") {
          r.innerHTML = `
...
<div style="animation:up .38s ease">
  ...
  ${enviado ? `<div class="sent-badge">✓ RESULTADOS ENVIADOS PARA A BIA</div>` : "<br>"}
  ...
  <div class="arch-main">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
      ...
      <span style="font-size:11px;font-weight:700;letter-spacing:.12em;color:${pa.c4};background:rgba(${rgb(pa.c4)},.12);padding:4px 12px">${pcts[0]}%</span>
    </div>
    <span class="arch-sym" style="color:${pa.c4}">${pa.sym}</span>
    <h3 class="os" style="font-size:clamp(1.9rem,7vw,2.8rem);color:${pa.c4};line-height:1;margin-bottom:6px">${pa.name}</h3>
    <p class="arch-tg">${pa.tag}</p>
    <p class="arch-desc">${pa.desc}</p>
    <p class="arch-dt"><span style="color:${pa.c4};font-weight:700">VOZ —</span> ${pa.voz}</p>
    <p class="arch-dt"><span style="color:${pa.c4};font-weight:700">PONTO FORTE —</span> ${pa.forte}</p>
    <p class="arch-dt" style="margin-bottom:0"><span style="color:${pa.c4};font-weight:700">ATENÇÃO —</span> ${pa.desafio}</p>
    <div class="sb-t"><div class="sb-f" style="width:${pcts[0]}%;background:${pa.c4}"></div></div>
  ...
  <div class="grid2">
    ...
    <div class="arch-sup">
      ...
      <p class="tiny" style="color:${a.c4};margin-bottom:10px">APOIO ${i + 1}</p>
      <span class="arch-sym-sm" style="color:${a.c4}">${a.sym}</span>
      <h4 class="os" style="font-size:1.25rem;color:var(--c);line-height:1;margin-bottom:4px">${a.name}</h4>
      <p class="arch-tg">${a.tag}</p>
      <p class="arch-desc" style="font-size:12px">${a.desc}</p>
      <div>${a.kw.map((k) => `<span class="kw">${k}</span>`).join("")}</div>
      <div class="sb-t" style="margin-top:10px"><div class="sb-f" style="width:${pcts[i + 1]}%;background:${a.c4}"></div></div>
